...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[POST /api/orgs] unhandled error:', err)
    return NextResponse.json(
      { data: null, error: { code: 'INTERNAL_ERROR', message: err instanceof Error ? err.message : 'Unknown error', statusCode: 500 } } satisfies ApiError,
      { status: 500 }
    )
  }
}
